...
#pragma GCC diagnostic ignored "-Wpedantic" // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=61491
	...
	template <> struct Screen<HW_CLASSWIZ_II>::Sprite
	{
		enum Enum : unsigned
		{
			SPR_PIXEL,
	...
	template <> struct Screen< HW_CLASSWIZ >::Sprite
	{
		enum Enum : unsigned
		{
			SPR_PIXEL,
	...
	template <> struct Screen< HW_ES_PLUS >::Sprite
    {
		enum Enum : unsigned
		{
			SPR_PIXEL,
...
#pragma GCC diagnostic pop
	...
	template<> const SpriteBitmap Screen<HW_CLASSWIZ_II>::sprite_bitmap[Screen< HW_CLASSWIZ_II >::Sprite::Enum::SPR_MAX] = {
		{"rsd_pixel",    0,    0},
		{"rsd_s",     0x01, 0x01},
		{"rsd_math",  0x01, 0x03},
		{"rsd_d",     0x01, 0x04},
	...
	template<> const SpriteBitmap Screen<HW_CLASSWIZ>::sprite_bitmap[Screen< HW_CLASSWIZ >::Sprite::Enum::SPR_MAX] = {
		{"rsd_pixel",    0,    0},
		{"rsd_s",     0x01, 0x00},
		{"rsd_a",     0x01, 0x01},
		{"rsd_m",     0x01, 0x02},
	...
	template<> const SpriteBitmap Screen<HW_ES_PLUS>::sprite_bitmap[Screen< HW_ES_PLUS >::Sprite::Enum::SPR_MAX] = {
		{"rsd_pixel",    0,    0},
		{"rsd_s",     0x10, 0x00},
		{"rsd_a",     0x04, 0x00},
		{"rsd_m",     0x10, 0x01},
	...
	{
		auto constexpr SPR_MAX = Screen< hardware_id >::Sprite::Enum::SPR_MAX;
	...
	template<HardwareId hardware_id> void Screen<hardware_id>::Frame()
	{
		...
		if (enable_status)
		{
			for (int ix = Sprite::Enum::SPR_PIXEL + 1; ix != Sprite::Enum::SPR_MAX; ++ix)
			{
		...
		if (enable_dotmatrix)
		{
			static constexpr auto SPR_PIXEL = Sprite::Enum::SPR_PIXEL;
